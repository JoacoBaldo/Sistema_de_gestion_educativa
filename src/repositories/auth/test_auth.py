...
def test_create_and_verify_token_flow():
    ...
    create_usecase = CreateTokenUseCase(token_response=token_service, cache_response=cache)
    verify_usecase = VerifyTokenUseCase(token_request=token_service, cache_request=cache)
    
    # Generate token
    token = create_usecase.execute(user_id=10, username="jdoe", email="jdoe@university.edu")
    assert len(token) > 0
...
def test_token_overwriting_single_session_security():
    ...
    token_a = create_usecase.execute(user_id=10, username="jdoe", email="jdoe@university.edu")
    ...
    token_b = create_usecase.execute(user_id=10, username="jdoe", email="jdoe@university.edu")
...
def test_token_expiration_ttl():
    ...
    token = create_usecase.execute(user_id=10, username="jdoe", email="jdoe@university.edu", ttl_seconds=1)
