...
(function () {
  ...
  document.getElementById("at-generate")?.addEventListener("click", async () => {
    ...
    const tokenGenerado = "qr-test-" + Math.random().toString(36).substring(2, 10);
    const urlAsistencia = `${window.location.origin}/asistencia/qr/${tokenGenerado}`;

    if (qrContainer) {
      qrContainer.innerHTML = "";
      qrContainer.classList.remove("at-qr-placeholder");
    }
    ...
    if (QRCodeLib) {
      ...
      showToast("Error: La librería de QR no está cargada.");
    }
  });
